...
namespace COTES.ISTOKDataUpdate.DBUpdate.Updaters
{
    ...
    class AddUserFullNameNPositionColumnsUpdater : IDBUpdater
    {
        ...
        public bool Update(CurrentService service)
        {
            using (MyDBdata dbwork = new MyDBdata(ServerType.MSSQL))
            {
                ...
                try
                {
                    const String addFullNameColumnQuery = "ALTER TABLE [users] ADD	[fullname] [nvarchar](1024) NULL";
                    const String addPositionColumnQuery = "ALTER TABLE [users] ADD	[position] [nvarchar](256) NULL";

                    transactionID = dbwork.StartTransaction();

                    dbwork.ExecSQL(transactionID, addFullNameColumnQuery, null);

                    dbwork.ExecSQL(transactionID, addPositionColumnQuery, null);
