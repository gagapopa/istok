...
{
    [Serializable]
    public class MultiDimensionalTable : IManualSerializable
    {
        ...
        public void AddDimension(string name, string measure, double value)
        {
            ...
            if (!double.IsNaN(value))
            {
                if (data == null || data.Count == 0)
                    new_data.Add(value, 0d);
                else
                    new_data.Add(value, data);
            }
            //if (dimentionInfo.Length == 0)
            //    lstDimensionInfo.Add(new DimensionInfo());
            //else
        ...
        public void RemoveDimension(DimensionInfo dInfo)
        {
            ...
            if (dimentionInfo.Length == 2)
            {
                dimensions = new DimensionInfo[0];
            }
            else
            {
            ...
            try
            {
                if (dimensions.Length > 0)
                {
                    ...
                    if (!ptr.MoveNext()) throw new Exception();
        ...
        {
            return GetDimension(CommonData.Approx.None, coordinates);
        }
        /// <summary>
        /// Выдает массив координат измерения.
        /// </summary>
        ...
        public double[] GetDimension(CommonData.Approx approx, params double[] coordinates)
        {
            ...
            switch (approx)
            {
                ...
                case CommonData.Approx.Linear:
                    ...
                    break;
                case CommonData.Approx.Quadratic:
                    break;
            }
        ...
        public void FromBytes(byte[] bytes)
        {
            using (MemoryStream stream = new MemoryStream(bytes))
            {
                using (BinaryReader sr = new BinaryReader(stream))
                {
                    ...
                    {
                        b = sr.ReadInt32();
                        ptr = new DimensionInfo();
                        ptr.FromBytes(sr.ReadBytes(b));
                        lstDimensions.Add(ptr);
                    }
                    dimentionInfo = lstDimensions.ToArray();
                    data.Clear();
                    data = (Dictionary<double, object>)ReadDictionary(sr);
                }
            }
        }
        ...
        public byte[] ToBytes()
        {
            ...
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                {
                    sw.Write(dimentionInfo.Length);
                    foreach (var item in dimentionInfo)
                    {
                        res = item.ToBytes();
                        sw.Write(res.Length);
                        sw.Write(res);
                    }
                    WriteDictionary(sw, data);
                }
                res = stream.ToArray();
            }
            return res;
        ...
        protected void WriteDictionary(BinaryWriter sw, IDictionary idata)
        {
            if (sw == null) throw new ArgumentNullException("sw");
            if (idata == null) throw new ArgumentNullException("idata");

            sw.Write(idata.Count);
            foreach (var item in idata.Keys)
            {
                sw.Write((double)item);
                if (idata[item] is double)
                {
                    sw.Write((int)DataType.Double);//признак того, что данные - double
                    sw.Write((double)idata[item]);
                }
                else
                    if (idata[item] is IDictionary)
                    {
                        sw.Write((int)DataType.Dictionary);//признак того, что данные - словарик
                        WriteDictionary(sw, (IDictionary)idata[item]);
                    }
                    else
                        throw new NotSupportedException("что это за?");
            }
        }
        protected IDictionary ReadDictionary(BinaryReader sr)
        {
            IDictionary res;
            double key;
            object value;
            ...
            count = sr.ReadInt32();
            res = new Dictionary<double, object>();
            for (int i = 0; i < count; i++)
            {
                key = sr.ReadDouble();
                type = sr.ReadInt32();
    ...
    /// </summary>
    [Serializable]
    public class DimensionInfo : ICloneable, IManualSerializable
    {
