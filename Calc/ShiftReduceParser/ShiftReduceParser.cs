...
namespace gppg
{
	public abstract class ShiftReduceParser<YYSTYPE,YYLTYPE> 
        where YYLTYPE : IMerge<YYLTYPE>, new ()
    {
        ...
        protected void Reduce(int rule_nr)
        {
            ...
            else
            {
                if (rule.rhs.Length == 0)
                {
                    ...
                    yyloc = (scanner.yylloc != null && lastL != null ? 
                        scanner.yylloc.Merge(lastL) : 
                        default(YYLTYPE));
                }
                else
                {
        ...
        public void ReportError()
        {
            ...
            this.setAliases(next);
            //errorMsg.AppendFormat("Syntax error, unexpected {0}", TerminalToString(next));
            errorMsg.AppendFormat("Синтаксичекая ошибка, неожиданный {0}", TerminalToString(next));
            this.clearAliases(next);
            ...
            if (current_state.parser_table.Count < 7)
            {
                ...
                {
                    if (first)
                        errorMsg.Append(", ожидается ");//", expecting ");
                    else
                        errorMsg.Append(", или ");//", or ");
        ...
        protected virtual void setAliases(int next) { }

        protected virtual void clearAliases(int next) { }
